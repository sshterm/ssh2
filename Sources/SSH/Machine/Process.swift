...
public extension SSH {
    func getSystemProcess() async -> [SystemProcess] {
        let ret1: [SystemProcess] = await findSystemProcess()
        sleep(1)
        var ret2: [SystemProcess] = await findSystemProcess()
        ...
        for i in 0...cout {
            ...
            ret2[i].percent = SystemProcess.calculatePercent(t1: ret1[i], t2: ret2[i], delta: 1)
        }
        return ret2
    }
    
    func findSystemProcess() async -> [SystemProcess] {
        ...
        for line in lines {
            ...
            v.user = Double(fields[13]) ?? 0.0
            v.system = Double(fields[14]) ?? 0.0
            v.childrenUser = Double(fields[15]) ?? 0.0
            v.childrenSystem = Double(fields[16]) ?? 0.0
            v.iowait = Double(fields[16]) ?? 0.0
            ret.append(v)
        }
